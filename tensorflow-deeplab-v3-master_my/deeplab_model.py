...
def deeplabv3_model_fn(features, labels, mode, params):
    ...
    pred_decoded_labels = tf.py_func(preprocessing.decode_labels,
                                     ...
                                     tf.uint8)
    print('check_decoded_labels', pred_decoded_labels)
    print(tf.shape(pred_decoded_labels))
    ...
    print('check_gt_decoded_labels',gt_decoded_labels)
    print(tf.shape(gt_decoded_labels))
    # argmax_labels = tf.argmax(labels)
    # print('check_tf_labels',argmax_labels)
    labels = tf.squeeze(labels, axis=3)  # reduce the channel dimension.
    print('check_label', labels)
    print(tf.shape(labels))
    logits_by_num_classes = tf.reshape(logits, [-1, params['num_classes']])
    print('check_logits_by_num_classes', logits_by_num_classes)
    labels_flat = tf.reshape(labels, [-1, ])
    print('check_labels_flat', labels_flat)

    valid_indices = tf.to_int32(labels_flat <= params['num_classes'] - 1)
    print('check_valid_indices', valid_indices)
    valid_logits = tf.dynamic_partition(logits_by_num_classes, valid_indices, num_partitions=2)[1]
    print('check_valid_logits', valid_logits)
    valid_labels = tf.dynamic_partition(labels_flat, valid_indices, num_partitions=2)[1]
    print('check_valid_labels', valid_labels)

    preds_flat = tf.reshape(pred_classes, [-1, ])
    print('check_preds_flat', preds_flat)
    valid_preds = tf.dynamic_partition(preds_flat, valid_indices, num_partitions=2)[1]
    print('check_valid_preds', valid_preds)
    confusion_matrix = tf.confusion_matrix(valid_labels, valid_preds, num_classes=params['num_classes'])
    print('check_confusion_matrix', confusion_matrix)

    predictions['valid_preds'] = valid_preds
    print('check_ predictions_valid_preds', predictions['valid_preds'])
    predictions['valid_labels'] = valid_labels
    print('check_ predictions_valid_labels', predictions['valid_labels'])
    predictions['confusion_matrix'] = confusion_matrix
    print('check_ predictions_confusion_matrix', predictions['confusion_matrix'])
    # 원래loss함수
